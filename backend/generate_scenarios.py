...
Reads data/raw/aggregated_entity_properties.json — a FLAT list of entities
...
Output:
    data/scenarios/scenario_<01, 02, ...>_<complexity>.json
"""
...
def load_entities(path: Path):
    ...
    if entities is None:
        raise ValueError(f"Expected an 'entities' key in {path}")
    return entities
...
def describe(component_graph: nx.DiGraph, complexity: str):
    ...
    edge_types_present = sorted({d["edge_type"] for _, _, d in component_graph.edges(data=True)})
    has_cycle = len(list(nx.simple_cycles(component_graph))) > 0
...
def build_scenario_payload(component_graph: nx.DiGraph, index: int, complexity: str):
    ...
    edges = [
        {"source": s, "target": t, "amount": d["amount"], "edge_type": d["edge_type"]}
        for s, t, d in component_graph.edges(data=True)
    ]
